...
/// </summary>
public partial class TempTileController(
    ICoreServerAPI sapi,
    MbTilesStorage mbTilesStorage)
{
    private static readonly Regex TempTilePathRegex = MyRegex();
    ...
    /// </summary>
    public async Task ServeTempTile(HttpListenerContext context, string path)
    {
        try
        {
            // Parse OpenLayers grid coordinates from path: /tiles/{zoom}/{gridX}_{gridY}.png
            var match = TempTilePathRegex.Match(path);
            if (!match.Success)
            {
            ...
            var result = await mbTilesStorage.GetTempTileAsync(gridX, gridY);
    ...
    /// </summary>
    public static bool IsTempTilePath(string path)
    {
        return TempTilePathRegex.IsMatch(path);
    }
    ...
    [GeneratedRegex(@"^/temp-tiles/(-?\d+)_(-?\d+)\.png$", RegexOptions.IgnoreCase | RegexOptions.Compiled, "en-US")]
