...
export function createWorldLayer(): TileLayer<XYZ> {
  ...
  console.log('[createWorldLayer] Creating tile layer with tile grid:', {
    resolutions: tileGrid.getResolutions(),
    extent: tileGrid.getExtent(),
    origin: tileGrid.getOrigin(0) // Get origin at zoom level 0
  });
  ...
  const source = new XYZ({
    ...
    tileUrlFunction: (tileCoord) => {
      if (!tileCoord) {
        console.warn('[Tile URL] No tile coord provided');
        return '';
      }
      const z = tileCoord[0];
      const x = tileCoord[1];
