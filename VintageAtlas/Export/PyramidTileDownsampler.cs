    ...
    ITileGenerator generator)
{
    ...
    public async Task<byte[]?> GenerateTileByDownsamplingAsync(int zoom, int tileX, int tileZ)
    {
        ...
        try
        {
            ...
            //  tiles will appear as transparent/empty areas
            // ═══════════════════════════════════════════════════════════════
            ...
            switch (missingCount)
            {
                case 4:
                    ...
                    sapi.Logger.Debug($"[VintageAtlas] All source tiles missing for {zoom}/{tileX}_{tileZ}, skipping");
                    return null;
                case > 0:
                    sapi.Logger.Debug($"[VintageAtlas] Downsampling with {4 - missingCount}/4 tiles available for {zoom}/{tileX}_{tileZ}");
                    break;
            }
