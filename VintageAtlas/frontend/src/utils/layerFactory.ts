...
export function createVectorLayer(options: LayerOptions) {
  ...
  const source = new VectorSource({
    url,
    format: new GeoJSON({
      // WebCartographer approach: Use default Web Mercator
      // dataProjection: where the data comes from (our backend sends EPSG:3857)
      // featureProjection: where to display it (also EPSG:3857)
 ...
 */
export function createExploredChunksLayer(visible = false, projection?: any) {
  return createVectorLayer({
